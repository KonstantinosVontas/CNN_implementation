...
#with open('./data_generate/overlap_curvature_h1_50k_4_to_40_val.csv') as file:
with open('./not_normalized_data/overlap_curvature_h1_200000_4_to_40_train.csv') as file:
...
with open('./not_normalized_data/overlap_curvature_h1_40000_4_to_40_val.csv') as file:
...
with open('./not_normalized_data/overlap_curvature_h1_40000_4_to_40_test.csv') as file:
    reader = csv.reader(file)
    test_data = []
    for row in reader:
...
val_data = val_data[0:60000][:]
test_data = test_data[0:60000][:]
...
class myData(Dataset):
    ...
    def __init__(self, path):
        ...
        self.input_data = self.input_data.reshape(len(self.input_data), 1, 3, 3, 3)
...
class CNN_Curv(torch.nn.Module):
    def __init__(self, size_h_layers, activations):
        super(CNN_Curv, self).__init__()

        self.net = nn.Sequential(
            nn.Conv3d(1, 32, 3, stride=1),
            nn.LayerNorm((32, 1, 1, 1)),
            nn.Flatten(start_dim=-4, end_dim=-1),
            nn.CELU(),
            nn.Linear(32, 16),
            nn.CELU(),
            nn.Linear(16, 1),
...
model = CNN_Curv([1, 32, 32, 16, 1], [nn.CELU(), nn.Linear(32, 16), nn.CELU(), nn.Linear(16, 1)])
#model = CNN_Curv([27, 20, 10, 5, 1], [nn.CELU(), nn.CELU(), nn.CELU(), nn.Identity()])
...
for epoch in range(n_epochs):
    ...
    for i, data1 in enumerate(train_dataloader, start=0):  # train_loader
        ...
        #print("The shape of inputs1 ", inputs1.shape)
        ...
        with torch.set_grad_enabled(is_train):
            ...
            #print("loss_train_normalized ", loss_train_normalized)
    ...
    if sum_validation_loss_normalized_iter < best_loss_from_validation:
        ...
        torch.save({
            ...
            'loss': loss_fn,
        }, f'./best_model_200k_data_{n_epochs}_LayerNorm_CELU_x2_Linear_batch_128.pth')
        # w = model.weight
...
plt.legend()
plt.title(f"Curvature Relative Error - 200k data, CNN([LayerNorm(), CELU_x2(), Linear()])", fontsize=7)
plt.savefig(f'plots_for_thesis/Curvature_Rel_error-200k_data_{n_epochs}_epochs_LayerNorm_CELU_x2_Linear_end_batch_128.pdf', dpi=700,  bbox_inches='tight')
plt.savefig(f'./plots_for_thesis/Curvature_Rel_error-200k_data_{n_epochs}_epochs_LayerNorm_CELU_x2_Linear_end_batch_128.eps',
            dpi=700, format='eps', bbox_inches='tight')
plt.savefig(f'./plots_for_thesis/Curvature_Rel_error-200k_data_{n_epochs}_epochs_LayerNorm_CELU_x2_Linear_end_batch_128.png',
...
plt.legend()
plt.title(f"Curvature Absolute error - 200k data, CNN([LayerNorm(), CELU_x2(), Linear()])", fontsize=7)
plt.savefig(f'plots_for_thesis/Curvature_Abs_error-200k_data_{n_epochs}_epochs_LayerNorm_CELU_x2_Linear_end_batch_128.pdf', dpi=700,  bbox_inches='tight')
plt.savefig(f'./plots_for_thesis/Curvature_Abs_error-200k_data_{n_epochs}_epochs_LayerNorm_CELU_x2_Linear_end_batch_128.eps',
            dpi=700, format='eps', bbox_inches='tight')
plt.savefig(f'./plots_for_thesis/Curvature_Abs_error-200k_data_{n_epochs}_epochs_LayerNorm_CELU_x2_Linear_end_batch_128.png',
            dpi=1200, format='png', bbox_inches='tight')
plt.show()
...
def myPlot():
    ...
    plt.legend()
    plt.title("Curvature Loss - 200k data, CNN([LayerNorm(), CELU_x2(), Linear()])", fontsize=7)
    plt.savefig(f'./plots_for_thesis/Curvature - 200k data, ADAM,_{n_epochs}_epochs_LayerNorm_CELU_x2_Linear_end_batch_128.pdf',
                dpi=700, format='pdf', bbox_inches='tight')
    plt.savefig(f'./plots_for_thesis/Curvature - 200k data, ADAM,_{n_epochs}_epochs_LayerNorm_CELU_x2_Linear_end_batch_128.eps',
                dpi=700, format='eps', bbox_inches='tight')
    plt.savefig(f'./plots_for_thesis/Curvature - 200k data, ADAM,_{n_epochs}_epochs_LayerNorm_CELU_x2_Linear_end_batch_128.png',
               dpi=1200, format='png', bbox_inches='tight')
    plt.show()
...
# TRAINING
outfile = open(f"./lists_for_plots/200k_LayerNorm_total_loss_training_{n_epochs}_epoch_batch_128.csv", 'w')
out = csv.writer(outfile)
out.writerows(map(lambda x: [x], total_loss_training_epoch))
outfile.close()

# VALIDATION
outfile = open(f"./lists_for_plots/200k_LayerNorm_total_loss_validation_{n_epochs}_epoch_batch_128.csv", 'w')
out = csv.writer(outfile)
out.writerows(map(lambda x: [x], total_loss_validation_epoch))
outfile.close()

# Test
outfile = open(f"./lists_for_plots/200k_LayerNorm_total_loss_test_{n_epochs}_epoch_batch_128.csv", 'w')
out = csv.writer(outfile)
out.writerows(map(lambda x: [x], total_loss_testing_epoch))
outfile.close()

# PLOT x_axis_epoch - for y_axis_min_validation
with open(f"./lists_for_plots/200k_LayerNorm_x_axis_best_model_{n_epochs}_epoch_batch_128.csv", 'w') as f_output:
    f_output.write(str(x_axis_epoch))

# PLOT : y_axis_min_validation
with open(f"./lists_for_plots/200k_LayerNorm_y_axis_min_validation_{n_epochs}_epoch_batch_128.csv", 'w') as f_output:
...
# TRAINING
outfile = open("./lists_for_plots/200kLayerNorm_total_Relative_error_training_epoch.csv", 'w')
out = csv.writer(outfile)
out.writerows(map(lambda x: [x], total_train_denormalized_epoch_rel_error))
outfile.close()

# VALIDATION
outfile = open("./lists_for_plots/200k_LayerNorm_total_Relative_error_validation_epoch.csv", 'w')
out = csv.writer(outfile)
out.writerows(map(lambda x: [x], total_val_denormalized_epoch_rel_error))
outfile.close()

# Test
outfile = open("./lists_for_plots/200k_LayerNorm_total_Relative_error_test_epoch.csv", 'w')
out = csv.writer(outfile)
...
# TRAINING
outfile = open("./lists_for_plots/200k_LayerNorm_total_Absolute_error_training_epoch.csv", 'w')
out = csv.writer(outfile)
out.writerows(map(lambda x: [x], total_train_denormalized_epoch_abs_error))
outfile.close()

# VALIDATION
outfile = open("./lists_for_plots/200k_LayerNorm_total_Absolute_error_validation_epoch.csv", 'w')
out = csv.writer(outfile)
out.writerows(map(lambda x: [x], total_val_denormalized_epoch_abs_error))
outfile.close()

# Test
outfile = open("./lists_for_plots/200k_LayerNorm_total_Absolute_error_test_epoch.csv", 'w')
out = csv.writer(outfile)
...
df.to_csv(
    f"./plots_for_thesis/200k_LayerNorm_Celux2_Linear_end_{n_epochs}_epochs_All_Losses_Errors_batch_128.csv",
