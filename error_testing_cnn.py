...
checkpoint = torch.load("./best_model_300k_data_3000_LayerNorm_CELU_x2_Linear_batch_128.pth")
...
for i in range(4, 42, 2):
    ...
    plt.legend()
    plt.title(f"Curvature - Average relative error: {median_error:.4f}, for radius = {radius_value}, for {number_of_samples} samples, CNN([LinearNorm, CELU() x2, Linear()])",
        fontsize=6)
    plt.savefig(f'./Loss_testing_plots_best_model/Curvature_relative_error_{number_of_samples}_samples_radius_{radius_value}_CNN_300k_data.pdf', dpi=300,  bbox_inches='tight')
    plt.show()
    plt.close("all")
    ...
    plt.legend()
    plt.title(f"Curvature - Average absolute error: {median_absolute_error}, for radius = {radius_value}, for {number_of_samples} samples, CNN([LayerNorm(), CELU() x2, Linear()])",
        fontsize=6)
    plt.savefig(f'./Loss_testing_plots_best_model/Curvature_relative_error_{number_of_samples}_samples_radius_{radius_value}_CNN_300k_data.pdf', dpi=300,  bbox_inches='tight')
    plt.show()
    plt.close("all")
...
plt.legend()
plt.title(f"Curvature - Average relative error: {relative_error_all_median_error_values.item():.4f}, for {number_of_samples} samples for each radius, CNN([LayerNorm(), CELU() x2, Linear()])", fontsize=7)
plt.savefig(f'./Loss_testing_plots_best_model/Average_Relative_error_over_Radius_for_{number_of_samples}_samples_LayerNorm_CELU_x2_Linear_end_300k_data.pdf', dpi=300,  bbox_inches='tight')
plt.show()
plt.close()
...
#all_median_absol_error_values = list(map(lambda x: str(x), all_median_absol_error_values.round(4)))
...
plt.legend()
plt.title(f"Curvature - Average absolute error: {average_of_all_median_absol_error_values:.4f}, for {number_of_samples} samples for each radius, CNN([LayerNorm(), CELU() x2, Linear()])", fontsize=7)
plt.savefig(f'./Loss_testing_plots_best_model/Average_Absolute_error_over_Radius_for_{number_of_samples}_samples_LayerNorm_CELU_x2_Linear_end_300k_data.pdf', dpi=300,  bbox_inches='tight')
plt.show()
plt.close()
